    ...
    def forward(self, inputs, noise_scale=0.667):
        ...
        if self.style_diffusion:
            cond = self.style_diffusion.sampler(
                ...
                embedding_scale=1.0,
            ).reshape(x.size(0), self.style_dim, 1)
        else:
            cond = None

        x_frame, p_attn, duration, y_mask = self.duration_handler.infer(x, x_mask, cond=cond)

        x, m_p, logs_p = self.frame_prior_network(x, y_mask, cond=cond)
        pitch_pred = self.pitch_predictor(x * y_mask, y_mask, cond=cond)
        log_cf0_pred, vuv_logit_pred = pitch_pred.chunk(2, dim=1)
        ...
        z = self.flow(z_p * y_mask, y_mask, cond=cond)
        o = self.vocoder(z * y_mask, f0, cond=cond)
        return o, (p_attn,)

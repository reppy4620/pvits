...
from x_vits.utils.const import LRELU_SLOPE
...
class SingleScaleDiscriminator(torch.nn.Module):
    def __init__(self):
    ...
    def forward(self, y, y_hat):
        ...
        y_d_r, fmap_r = self.d(y)
        y_d_g, fmap_g = self.d(y_hat)
        y_d_rs.append(y_d_r)
        fmap_rs.append(fmap_r)
        y_d_gs.append(y_d_g)
        fmap_gs.append(fmap_g)
...
class DiscriminatorR(nn.Module):
    def __init__(self, resolution, mult=1):
        ...
        assert len(self.resolution) == 3, "MRD layer requires list with len=3, got {}".format(
            self.resolution
        )
        self.d_mult = mult
        ...
        self.conv_post = weight_norm(nn.Conv2d(int(32 * self.d_mult), 1, (3, 3), padding=(1, 1)))
